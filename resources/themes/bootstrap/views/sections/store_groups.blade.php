...
<div class="container px-4 py-5 py-lg-7 mx-auto">
    <div class="text-center mb-4 mb-lg-5 mx-auto">
        <h2 class="h2 fw-bold">{{ __('store.store_subheading') }}</h2>
        <p class="mt-2 text-muted">{{ __('store.subtitle') }}</p>
    </div>
            ...
            @foreach($row as $group)
                <div class="col d-flex align-items-stretch">
                    <div class="card mt-3 flex-grow-1">
                        @if ($group->image)
                            <img src="{{ Storage::url($group->image) }}" class="mx-auto {{ $group->useImageAsBackground() ? 'w-100 h-100' : 'w-32 h-32 my-2' }}" alt="{{ $group->trans('name') }}">
                        @endif
                        <div class="card-body d-flex flex-column">
                            <h5 class="card-title">{{ $group->trans('name') }}

                                @if ($group->pinned)
                                    <span class="badge border border-primary-subtle">{{ __('store.pinned') }}</span>
                                @endif
                            </h5>
