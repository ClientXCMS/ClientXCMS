        ...
        <div class="card">
            <div class="card-heading">
                ...
                <div>
                    ...
                    @endif
                    @if (isset($exportEnabled) && $exportEnabled)
                        <button class="btn btn-primary" type="button" data-hs-overlay="#client-invoice-export-overlay">
                            <i class="bi bi-download"></i> {{ __('client.invoices.export.button') }}
                        </button>
                    @endif
...
@if (isset($exportEnabled) && $exportEnabled)
<div id="client-invoice-export-overlay"
    ...
    <div class="flex justify-between items-center py-3 px-4 border-b dark:border-gray-700">
        ...
        <button type="button"
                ...
                data-hs-overlay="#client-invoice-export-overlay">
    ...
    <div class="p-4">
        @php
            $invoiceStatusOptions = collect($filters)->mapWithKeys(fn ($status) => [$status => __('global.states.'.$status)])->all();
            $selectedExportStatuses = old('status', $invoiceFilters['status'] ?? ['paid']);
        @endphp
        <form method="POST" action="{{ route('front.invoices.export') }}">
            ...
            @if(count($invoiceStatistics['available_currencies']) > 1)
                @include('admin/shared.select', [
                    ...
                    'options' => array_combine($invoiceStatistics['available_currencies'], $invoiceStatistics['available_currencies']),
                    'value' => old('currency', $invoiceStatistics['currency']),
                ])
            @elseif(!empty($invoiceStatistics['currency']))
                <input type="hidden" name="currency" value="{{ $invoiceStatistics['currency'] }}">
            @endif
            @csrf
            @include('admin/shared.select', [
                'name' => 'format',
