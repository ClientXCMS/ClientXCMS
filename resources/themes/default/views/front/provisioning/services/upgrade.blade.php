                        ...
                        <div>
                            ...
                            @if (setting('checkout.toslink'))
                                <p class="text-sm text-warning">
                                    {{ __('client.services.upgrade.upgrade_notice') }}<a href="{{ setting('checkout.toslink') }}" class="text-primary">{{ __('store.checkout.terms') }}.</a>
                                </p>
                            @endif
                        </div>
                    </div>
                            ...
                            <div class="hs-dropdown">
                                ...
                                <div class="hs-dropdown-menu transition-[opacity,margin] duration hs-dropdown-open:opacity-100 opacity-0 hidden min-w-[15rem] bg-white shadow-md rounded-lg dark:bg-gray-800 dark:border dark:border-gray-700 dark:divide-gray-700 after:h-4 after:absolute after:-bottom-4 after:start-0 after:w-full before:h-4 before:absolute before:-top-4 before:start-0 before:w-full" aria-labelledby="hs-dropdown-default">
                                    @foreach ($gateways as $gateway)
                                        <a class="flex items-center gap-x-3.5 py-2 px-3 rounded-lg text-sm text-gray-800 hover:bg-primary-light focus:outline-none focus:bg-primary-light dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-gray-300 dark:focus:bg-gray-700" href="{{ route('front.services.renew', ['service' => $service, 'gateway' => $gateway->uuid]) }}">
                                            {{ $gateway->name }}
                                        </a>
                                    @endforeach
                                </div>
