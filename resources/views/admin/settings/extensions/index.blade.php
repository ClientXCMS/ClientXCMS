...
@php
    ...
    $newExtensions = $allExtensions
        ->filter(fn($ext) => isset($ext->api['tags']) && collect($ext->api['tags'])->contains('slug', 'new'))
        ->take(4);
    $popularExtensions = $allExtensions
        ->filter(fn($ext) => isset($ext->api['tags']) && collect($ext->api['tags'])->contains('slug', 'popular'))
        ->take(4);
@endphp
            ...
            <div class="relative z-10">
                ...
                <div class="mt-8">
                    <div class="relative max-w-2xl">
                        ...
                        <input type="text" id="extension-search"
        ...
        @if (!empty($groups))
            <div class="mb-8">
                <div class="flex flex-wrap items-center gap-2 p-1.5 bg-gray-100 dark:bg-slate-800 rounded-xl w-fit">
                    <button
                        class="main-tab-btn active px-6 py-3 rounded-lg text-sm font-semibold transition-all duration-200 bg-white dark:bg-slate-700 text-indigo-600 dark:text-indigo-400 shadow-sm"
                        data-tab="installed">
                        <i class="bi bi-collection-fill mr-2"></i>{{ __('extensions.settings.tabs.my_extensions') }}
                        <span
                            class="ml-2 px-2 py-0.5 bg-indigo-100 dark:bg-indigo-900/50 text-indigo-600 dark:text-indigo-400 rounded-full text-xs">{{ $installedExtensions->count() }}</span>
                    </button>
                    <button
                        class="main-tab-btn px-6 py-3 rounded-lg text-sm font-semibold transition-all duration-200 text-gray-600 dark:text-gray-400 hover:text-indigo-600 dark:hover:text-indigo-400"
                        data-tab="discover">
                        <i class="bi bi-compass-fill mr-2"></i>{{ __('extensions.settings.tabs.discover') }}
                        <span
                            class="ml-2 px-2 py-0.5 bg-gray-200 dark:bg-slate-600 text-gray-600 dark:text-gray-400 rounded-full text-xs">{{ $allExtensions->count() }}</span>
                    </button>
                    <button
                        class="main-tab-btn px-6 py-3 rounded-lg text-sm font-semibold transition-all duration-200 text-gray-600 dark:text-gray-400 hover:text-indigo-600 dark:hover:text-indigo-400"
                        data-tab="themes">
                        ...
                        <span
                            class="ml-2 px-2 py-0.5 bg-gray-200 dark:bg-slate-600 text-gray-600 dark:text-gray-400 rounded-full text-xs">{{ $themes->count() }}</span>
                    </button>
                </div>
            </div>
    ...
    <div id="tab-discover" class="tab-content {{ $installedModulesAddons->count() > 0 ? 'hidden' : '' }}">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-10">
            ...
            <div id="section-new">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-10">
                        ...
                        <div id="section-new">
                            <div class="flex items-center gap-3 mb-5">
                                ...
                                <h2 class="text-xl font-bold text-gray-900 dark:text-white">
                                    {{ __('extensions.settings.sections.new') }}</h2>
                            </div>
                            <div class="space-y-3">
                                @foreach ($newExtensions as $extension)
                                    @include('admin.settings.extensions._card-compact', [
                                        'extension' => $extension,
                                    ])
                                @endforeach
                            </div>
                    ...
                    @if ($popularExtensions->count() > 0)
                        <div id="section-popular">
                            <div class="flex items-center gap-3 mb-5">
                                <div
                                    class="w-10 h-10 bg-gradient-to-br from-rose-400 to-pink-500 rounded-xl flex items-center justify-center shadow-lg">
                                    <i class="bi bi-fire text-white text-lg"></i>
                                </div>
                                <h2 class="text-xl font-bold text-gray-900 dark:text-white">
                                    {{ __('extensions.settings.sections.popular') }}</h2>
                            </div>
                            <div class="space-y-3">
                ...
                <div class="mb-6">
                    <div class="flex items-center justify-between mb-5">
                        <div class="flex items-center gap-3">
                            <div
                                class="w-10 h-10 bg-gradient-to-br from-indigo-400 to-purple-500 rounded-xl flex items-center justify-center shadow-lg">
                                <i class="bi bi-grid-3x3-gap text-white text-lg"></i>
                            </div>
                            <h2 class="text-xl font-bold text-gray-900 dark:text-white">
                                {{ __('extensions.settings.sections.all_extensions') }}</h2>
                        </div>
                ...
                <div id="no-results"
                    class="hidden text-center py-16 bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700">
            ...
            <div id="tab-themes" class="tab-content hidden">
                <div class="mb-8">
                    <div class="flex items-center gap-3 mb-5">
                        ...
                        <h2 class="text-xl font-bold text-gray-900 dark:text-white">
                            {{ __('extensions.settings.sections.all_themes') }}</h2>
                        <span class="text-sm text-gray-500 dark:text-gray-400">({{ $themes->count() }}
                            {{ __('extensions.settings.themes_available') }})</span>
                    </div>
                </div>
                ...
                @else
                    <div
                        class="text-center py-16 bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700">
...
@section('scripts')
    <div id="bulk-toolbar" class="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 hidden">
        <div
            class="flex items-center gap-4 px-6 py-4 bg-white dark:bg-slate-800 rounded-2xl shadow-2xl border border-gray-200 dark:border-slate-700">
    ...
    <div id="extension-modal" class="fixed inset-0 z-50 hidden">
        ...
        <div class="flex items-center justify-center min-h-screen p-4">
            <div
                class="relative bg-white dark:bg-slate-900 rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-hidden">
                <button id="modal-close"
                    class="absolute top-4 right-4 z-10 p-2 rounded-lg bg-gray-100 dark:bg-slate-800 hover:bg-gray-200 dark:hover:bg-slate-700 transition-colors">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            dropzone.addEventListener('drop', (e) => {
                ...
                dropzone.innerHTML =
                    `<i class="bi bi-file-earmark-zip"></i> ${e.dataTransfer.files[0].name}`;
            });
