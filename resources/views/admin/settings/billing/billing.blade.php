...
<div class="card">
    ...
    <form action="{{ route('admin.settings.store.billing.save') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="grid gap-6 mt-4 md:grid-cols-3">
            ...
            <div>@include('admin/shared/select', ['name' => 'billing_country', 'label' => __('einvoicing.settings.seller_country'), 'help' => __('einvoicing.settings.seller_country_help'), 'options' => $billingCountries, 'value' => setting('billing_country', 'FR')])</div>
            ...
            <div>@include('admin/shared/select', ['name' => 'einvoicing_provider', 'label' => __('einvoicing.settings.provider'), 'options' => collect(app(\App\Services\Billing\ElectronicProviderRegistry::class)->all())->filter(fn ($provider) => in_array('b2b', $provider->capabilities(), true))->mapWithKeys(fn ($provider, $key) => [$key => strtoupper($key)])->all(), 'value' => setting('einvoicing_provider', 'local')])</div>
            <div>@include('admin/shared/select', ['name' => 'einvoicing_b2g_provider', 'label' => __('einvoicing.settings.b2g_provider'), 'options' => collect(app(\App\Services\Billing\ElectronicProviderRegistry::class)->all())->filter(fn ($provider) => in_array('b2g', $provider->capabilities(), true))->mapWithKeys(fn ($provider, $key) => [$key => strtoupper($key)])->all(), 'value' => setting('einvoicing_b2g_provider', app(\App\Services\Billing\ElectronicProviderRegistry::class)->has('chorus-pro') ? 'chorus-pro' : 'local')])</div>
