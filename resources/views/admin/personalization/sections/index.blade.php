                ...
                <div class="grid grid-cols-6 gap-4">
                    ...
                    <div class="col-span-6">
                        <div class="card">
                            ...
                            <div class="flex flex-wrap">
                                <ul class="w-full flex flex-col gap-2" data-button="#saveButton-{{ $uuid }}" data-url="{{ route('admin.personalization.sections.sort') }}" is="sort-list">
                                    @foreach ($active_page['sections'] as $section)
                                    <li class="hs-accordion bg-white border border-gray-200 shadow-sm rounded-xl dark:bg-slate-900 dark:border-slate-700 dark:shadow-slate-700/70 sortable-item" id="{{ $section->id }}">
                                        <div class="hs-accordion-toggle flex items-center justify-between p-4 cursor-pointer" id="hs-basic-heading-{{ $section->id }}">
                                            <div class="flex items-center gap-2 text-gray-800 dark:text-gray-200 font-medium">
                                                <i class="bi bi-grip-vertical text-gray-400 cursor-move handle"></i>
                                                <span>{{ $section->name ?? $section->uuid }}</span>
                                            </div>
                                            <div class="flex items-center gap-2" onclick="event.stopPropagation();">
                                                @if ($section->isPremium())
                                                <button type="button" class="btn btn-sm cursor-default">
                                                ...
                                                <form method="POST" action="{{ route($routePath . '.destroy', ['section' => $section]) }}">
                                                    @method('DELETE')
                                                    @csrf
                                                    <button class="btn btn-danger btn-sm" onclick="return confirm('Voulez-vous vraiment supprimer cette section ?')">
                                                        <i class="bi bi-trash"></i> {{ __('global.delete') }}
                                                    </button>
                                                </form>

                                                <button class="hs-accordion-toggle p-2 text-gray-500 hover:text-gray-700 dark:text-neutral-400 dark:hover:text-neutral-200" aria-controls="hs-basic-collapse-{{ $section->id }}">
                                        ...
                                        <div id="hs-basic-collapse-{{ $section->id }}" class="hs-accordion-content hidden w-full overflow-hidden transition-[height] duration-300" role="region" aria-labelledby="hs-basic-heading-{{ $section->id }}">
                                            <div class="px-4 pb-4 border-t border-gray-200 dark:border-neutral-700 pt-4">
                            ...
                            <div class="pb-4 px-5">
                                <ul class="w-full">
                                    ...
                                    <li class="flex items-center justify-between py-2 px-4 dark:border-neutral-700 rounded-lg mb-2">
                                        <div class="group relative flex flex-col h-full bg-white border border-gray-200 shadow-sm rounded-xl dark:bg-slate-900 dark:border-slate-700 dark:shadow-slate-700/70 w-full">
                                            <div class="absolute top-3 right-3 z-10">
                                                <form method="POST" action="{{ route($routePath . '.clone_section', ['section' => $section->uuid]) }}?active_page={{ $uuid }}">
                                                    @csrf
                                                    <button class="btn btn-primary btn-sm" type="submit">
                                                        Installer {{ $section->uuid }}
                                                    </button>
                                                </form>
                                            </div>
