...
@section('setting')
    <div class="card">
        <div class="card-heading">
            ...
            <div class="flex gap-2">
                ...
                <button type="button" class="btn btn-secondary text-sm" onclick="openMenuDrawerCreate()"
            ...
            <ul data-button="#saveButton"
                ...
                @foreach ($menus as $menu)
                    <li class="sortable-item {{ $menu->hasChildren() ? 'sortable-parent' : '' }}" id="{{ $menu->id }}"
                        data-menu-id="{{ $menu->id }}" data-name="{{ $menu->name }}" data-url="{{ $menu->url }}"
                        data-icon="{{ $menu->icon }}" data-badge="{{ $menu->badge }}"
                        data-description="{{ $menu->description }}" data-link-type="{{ $menu->link_type }}"
                        data-allowed-role="{{ $menu->allowed_role }}" data-parent-id="{{ $menu->parent_id }}">
                            ...
                            <div class="flex justify-between items-center">
                                ...
                                <div class="flex items-center gap-1">
                                    @if ($supportDropDropdown)
                                        <button type="button" class="btn-icon text-sm"
                                            onclick="openMenuDrawerCreate({{ $menu->id }})"
                                            data-hs-overlay="#menu-drawer"
                                    ...
                                    @endif
                                    <button type="button" class="btn-icon text-sm"
                                        onclick="openMenuDrawerEdit(this.closest('li'))" data-hs-overlay="#menu-drawer">
                                        <i class="bi bi-pencil"></i>
                                    </button>
                                    <a href="{{ route('admin.personalization.menulinks.show', ['menulink' => $menu->id]) }}"
                                        class="btn-icon text-sm" title="{{ __('global.show') }}">
                            ...
                            <ol is="sort-list2">
                                @foreach ($menu->children as $child)
                                    <li class="sortable-item {{ $child->children->count() > 0 ? 'sortable-parent' : '' }}"
                                        id="{{ $child->id }}" data-menu-id="{{ $child->id }}"
                                        data-name="{{ $child->name }}" data-url="{{ $child->url }}"
                                        data-icon="{{ $child->icon }}" data-badge="{{ $child->badge }}"
                                        ...
                                        <div class="card bg-white dark:bg-slate-900 dark:border-gray-800 ml-12">
                                            <div class="flex justify-between items-center">
                                                ...
                                                <div class="flex items-center gap-1">
                                                    @if ($supportDropDropdown)
                                                        <button type="button" class="btn-icon text-sm"
                                                            onclick="openMenuDrawerCreate({{ $child->id }})"
                                                            data-hs-overlay="#menu-drawer"
                                                            title="{{ __('personalization.menu_links.add_child') }}">
                                                            <i class="bi bi-plus-circle"></i>
                                                        </button>
    ...
    <div id="menu-drawer"
        class="hs-overlay hs-overlay-open:translate-x-0 hidden translate-x-full fixed top-0 end-0 transition-all duration-300 transform h-full max-w-lg w-full z-[80] bg-white border-s dark:bg-gray-800 dark:border-gray-700"
        tabindex="-1">
        ...
        <div class="p-4 overflow-y-auto h-[calc(100%-60px)]">
            ...
            <form id="menu-drawer-form" method="POST"
                action="{{ route('admin.personalization.menulinks.create', ['type' => 'bottom']) }}">
                @csrf
                ...
                <div class="space-y-4">
                    <div>
                        <label for="drawer-link-type"
                            class="block text-sm font-medium leading-6 text-gray-900 dark:text-gray-400 mt-2">{{ __('personalization.menu_links.link_type') }}</label>
                        <div class="mt-2">
                    ...
                    <div class="mt-2">
                        @include('admin/shared/input', [
                            'name' => 'icon',
                            'label' => __('personalization.icon'),
                            'id' => 'drawer-icon',
                            'attributes' => ['placeholder' => 'bi bi-house-door'],
                        ])
                    </div>
                    ...
                    <div class="mt-2">
                        @include('admin/shared/input', [
                            'name' => 'badge',
                            'label' => __('personalization.badge'),
                            'id' => 'drawer-badge',
                        ])
                    </div>

                    <div>
                        <label for="drawer-allowed-role"
                            class="block text-sm font-medium leading-6 text-gray-900 dark:text-gray-400 mt-2">{{ __('personalization.menu_links.allowed_role') }}</label>
                        <div class="mt-2">
    ...
    <script>
        ...
        var storeUrl = "{{ route('admin.personalization.menulinks.create', ['type' => 'bottom']) }}";
