    ...
    private function clearBasket(bool $clearBasket = true)
    {
        if ($clearBasket) {
            if ($this->getMetadata('basket') !== null) {
                ...
                if ($this->status === self::STATUS_PAID) {
                    ActionLog::log(ActionLog::BASKET_COMPLETED, $basket, $basket->id, $this->user_id, $this->user_id, ['invoice' => $this->invoice_number, 'amount' => formatted_price($this->total(), $this->currency()), 'currency' => $this->currency(), 'amount_decimal' => $this->total()]);
